...
class Tag extends Model
{
    ...
    {
        $stmt = $this->db->prepare('INSERT INTO tags (user_id, name) VALUES (:user_id, :name)');
        return $stmt->execute(['user_id' => $userId, 'name' => $name]);
    }

    public function update(int $id, int $userId, string $name): bool
    {
        $stmt = $this->db->prepare('UPDATE tags SET name = :name WHERE id = :id AND user_id = :user_id');
    ...
    {
        $stmt = $this->db->prepare('SELECT COUNT(*) FROM tags WHERE user_id = :user_id');
        $stmt->execute(['user_id' => $userId]);
